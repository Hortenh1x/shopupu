...
## E-026 — подтверждение владельца, исполнение retention, неделя эксплуатации (21.09.2026)

**Среда:** S, T (Testcontainers), P. **Требования:** LEG-01/02/05/06, OPS-01/06/08, AI-02, FINAL-04, Q-12/17/19. Владелец 21.09.2026: «я подтверждаю то, что ты решил сделать, можешь имплементировать, комитить и пушить на прод».

**Что подтверждено.** Решения (1)–(8) из §7 отчёта: оператор/контакт, retention, provenance изображений и шрифтов, SLO p95 ≤ 750 ms при 4 rps, публичный `/v3/api-docs`, минимальный live AI eval, Dependabot, ночной backup. Внешние подключения по-прежнему отложены владельцем.

**Найдено при исполнении подтверждения.** Страница «Data and privacy» обещала сроки хранения, которых код не исполнял: не было ни удаления аккаунтов без входа за 12 месяцев, ни срока для `audit_events`, ни 24-месячного лимита для псевдонимизированной истории удалённых аккаунтов, ни самого признака последнего входа. Обещание без исполнения — дефект LEG-06 («сроки хранения исполнимы»), поэтому реализовано: V25 `users.last_login_at` (существующим аккаунтам — дата последнего refresh-токена либо дата миграции, чтобы никто не был удалён раньше 12 месяцев *измеренной* неактивности); `RefreshTokenService.mint` фиксирует вход/регистрацию/продление сессии (запись не чаще раза в час); `RetentionProperties` (`RETENTION_*`, ISO-периоды, fail-fast на неположительных); `DataRetentionJob` ежедневно **04:10 UTC** — через 30 минут после ночного backup (03:40 UTC), чтобы удалённое ещё 14 дней жило в снимках, как и написано на странице: (1) `audit_events` старше 12 мес. пачками по 500; (2) CUSTOMER-аккаунты с `last_login_at` старше 12 мес. проходят тот же erasure, что GDPR-запрос, с событием `RETENTION_INACTIVE_ACCOUNT_ERASED`; ADMIN/MANAGER и неизвестный последний вход не трогаются; (3) аккаунты с `deleted_at` старше 24 мес. — удаление заказов (каскадом платежи/события/доставка/история) и строки `users`. Логи контейнера ограничены 10 × 50 MB (раньше — без предела). Дополнительно ночной AI-job теперь сначала восполняет недостающие embeddings (товары #1–#2 на P были без них с момента индексации; кнопка Backfill больше не единственный путь).

**Проверка.** `DataRetentionIT` (Testcontainers, 4/4): выдача сессии записывает `last_login_at`; событие 400 дней удалено, 300 дней — осталось; неактивный покупатель (380 дней) стёрт с аудит-событием, активный (200 дней), с неизвестным входом и неактивный ADMIN — не тронуты; удалённый 25 месяцев назад аккаунт с заказом и платежом исчез вместе с каскадами, удалённый месяц назад — сохранён. `./mvnw verify` — **323 unit + 98 IT PASS**, JaCoCo line 76,8 % / branch 63,4 % (первый прогон был испорчен параллельной перекомпиляцией из этой же сессии — `AuthService` отсутствовал в `target/classes` в момент сканирования; повтор чистый).

**Выпуск и live smoke.** Снимок БД до миграции (`~/backups/shopupu/<ts>-pre-v25`), V25 применена `shopupu_migrator` (323 из 323 живых аккаунтов получили baseline), образ `4a42f03` с label revision, health 200 через 18 s, ротация логов в `HostConfig.LogConfig`. Публичный сценарий: регистрация → профиль → корзина → checkout → LOCAL_PICKUP → платёж → симуляция → `PAID` → export → erasure (204, затем 401) — PASS; у нового аккаунта `last_login_at` записан. Попутно найдено: неверный метод/тип содержимого на известном пути (`GET /users/me`, `POST /orders`) уходил в catch-all как 500 с ERROR-логом — теперь 405/415 Problem Details с EN/DE текстами (`LocalizationTest`), выпущено следом.

**Итог выпуска 21.09:** backend `41dca7c` на P (health 200, revision label), `verify` 323 unit + 98 IT PASS. Тестовые аккаунты пробников стёрты через штатный erasure; один аккаунт без заказов (пароль пробника не сохранился после обрыва TLS в песочнице) удалён SQL вместе со своими audit-строками.

**Неделя эксплуатации P (14–21.09).** Ночные снимки 8/8 `OK` (03:40 UTC, 475 MB за все, диск 42 %); контейнеры без перезапусков 7 дней, health 200; еженедельный guarded harness в CI (расписание, понедельник) — PASS на `e06c18f`; Dependabot после включения графа: 10 PR (обновления actions и jjwt 0.13 — не применялись, ждут отдельного ревью совместимости; runtime-уязвимостей 0).

**Ограничения:** job проверен на синтетических датах в изолированной БД; на P ближайшие месяцы он ничего не удаляет (самые старые данные — июль 2026), первое реальное удаление audit-событий — лето 2027; ручная сессия со screen reader и production-capacity по-прежнему не выполнялись.
